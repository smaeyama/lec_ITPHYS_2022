...
int main(void){
  ...
  double t;         /* 時間 t */
  double xv[4], tmp[4]; /* 粒子の位置と速度を xv[0]=x,xv[1]=z,xv[2]=vx,xv[3]=vz とまとめた配列 */
  double k1[4], k2[4];  /* 同様に dx/dt, dz/dt, dvx/dt, dvz/dt をまとめた配列 */
  int i;
  /* 初期値の設定 */
  t = 0.0;
  xv[0] = 0.0; /* x(t=0) */
  ...
  for (t = dt; t <= 2.0; t=t+dt){
    ...
    for (i=0;i<4;i++) {
      tmp[i] = xv[i] + 0.5 * dt * k1[i];
    }
    calc_derivative(tmp,k2);
    for (i=0;i<4;i++) {
      xv[i] = xv[i] + dt * k2[i];
    }
    printf("%f %f %f %f %f\n", t, xv[0], xv[1], xv[2], xv[3]);
